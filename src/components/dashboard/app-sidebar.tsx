...
export function AppSidebar({ className }: AppSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 flex h-screen w-64 flex-col bg-sidebar text-sidebar-foreground",
        className
      )}
    >
      {/* Logo / Brand */}
      ...
      <div className="border-t border-sidebar-border p-4">
        {user ? (
          <div className="space-y-2">
            <div className="flex items-center gap-3 rounded-lg bg-sidebar-accent/30 px-3 py-2.5">
              ...
              <div className="flex min-w-0 flex-col">
                ...
                <span className="text-xs text-sidebar-muted">Conectado</span>
              </div>
            </div>
            <Button
              variant="ghost"
          ...
          <Link
            href="/login"
            className="flex items-center gap-3 rounded-lg bg-sidebar-accent/30 px-3 py-2.5 text-sm font-medium text-sidebar-foreground/70 transition-colors hover:bg-sidebar-accent/50 hover:text-sidebar-foreground"
          >
            <LogOut className="h-4 w-4" />
            Iniciar sesión
          </Link>
